...
file <- "C:\\Users\\pietr\\OneDrive - City St George's, University of London\\Documents\\CATTOLICA ANNO 24-25\\2 SEMESTRE\\(ASF) - Applied Statistics for Finance\\(ASF) - R\\Applied-Statistics-for-Finance\\Applied_statistics_for_finance.qmd" 
...
extract_r_code(file, "Applied_statistics_for_finance.R")
```
...
$$
S_t=x\cdot e^{\big(r-\frac{σ^2}{2}\big)\cdot t+σ\ W_t}
$$ 
with $t>0$ and $x=S_0$
...
$$
Var \frac12 \bigg(g(X) + g\Big(h(X)\Big)\bigg) < Var\Big(g(X)\Big)
$$
...
[Commmon question: Advantage and dissadvantages of MoM?]{style="color: darkorange; text-decorati on: underline; font-weight: bold;"} <br>
...
**Stock Price Simulation:** It simulates terminal stock prices under the Black-Scholes model 
$$
S_T=S_0 \cdot EXP\left\{(r− \frac12 σ^2)(T−t)+σ\sqrt{T−t}Z\right\}
$$ 
...
def objective_function(sigma, option_mkt_price, S, K, T, r, option_type):
    return abs((BS_option_price(S, K, T, r, sigma, option_type) - option_mkt_price))
```
...
file_path = Path("C:/Users/pietr/OneDrive/Documenti/CATTOLICA ANNO 24-25/2 SEMESTRE/(ASF) - Applied Statistics for Finance/(ASF) - R/ASF/Options_input.xlsx")
...
We can also use a lambda function which is more efficient and get the same results
...
1. **Apply a shock** to one input (e.g., underlying price $S_0$, volatility $\sigma$, time to maturity $T$, or interest rate $r$)
2. **Use the Greek** to linearly approximate the new price
3. **Compare** to the new price from re-running the full BS formula
...
print(implied_volatility(option_mkt_price, S0, K, T, r, option_type, initial_guess))
print(BS_greeks(100, 100, 1, 0.01, 0.2, "call", rounding = 4))
```
...
$x$ is the input variable (e.g., $S_0, \sigma, T$)
$\varepsilon$ is a small shift

This is a **finite difference** approach: generalizable and useful when analytical Greeks are unavailable

This method:

Works for **any option** (even exotic or path-dependent)
Still assumes that the function is smooth and approximable locally
...
print(f"Delta: {numerical_delta:.4f} vs Closed-Form: {greeks['delta']:.4f}")
print(f"Gamma: {numerical_gamma:.6f} vs Closed-Form: {greeks['gamma']:.6f}")
print(f"Vega:  {numerical_vega:.6f} vs Closed-Form: {greeks['vega']:.6f}")
print(f"Theta: {numerical_theta:.6f} vs Closed-Form: {greeks['theta']:.6f}")
print(f"Rho:   {numerical_rho:.6f} vs Closed-Form: {greeks['rho']:.6f}")
